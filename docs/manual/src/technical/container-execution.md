...
- **uid/gid** switched to the container user (read from `start` RPC
  ...
  `/etc/group`.
- **PTY** allocated when stdin is a TTY. The CLI sends the host
  terminal size as the initial PTY dimensions and forwards resize
  events (SIGWINCH).
...
— the supervisor relays all three streams over the same vsock RPC
connection that carries control calls. This means the user sees output as fast as
...
The protocol is pull-based rather than push-based because vsock
latency is negligible and pull gives natural backpressure — the guest
only sends when somebody is actively reading. Cap'n Proto RPC handles
the multiplexing of many in-flight calls over the single vsock
connection.
...
In PTY mode the CLI puts the host terminal into raw mode and hooks
SIGWINCH. Its `Stdin.read()` loop uses `tokio::select!` to multiplex
terminal reads with resize events into the single stream the
supervisor pulls.
...
with that code. When a signal kills the child, the exit code becomes
`128 + signum` per the Unix convention.
...
- airlock resolves the base env exactly once, at `airlock start`, not
  once per `exec`.
- Processes attached via `exec` inherit the same environment the main
  container process started with, which matches the user
