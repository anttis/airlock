...
VirtioFS shares must be mounted before the overlay can reference them.
Networking must be up before the proxy starts to accept connections.
Container mounts run last so file bind mounts win over earlier dir
bind mounts.
...
2. **VirtioFS shares** (`mount::virtiofs`):
   ...
   - One share per configured directory mount (tag `project` for the
     project dir and `dir_0`, `dir_1`, … for others).
...
4. **Project disk** (`disk::setup`) — formats the ext4 image if blank
   (`mkfs.ext4`), mounts it at `/mnt/disk`, and resizes the filesystem
   if the disk image was enlarged since the last boot. It also bumps
   ...
   Both are best-effort: the supervisor logs and swallows errors
   (including hypervisors that don't pass `VIRTIO_BLK_F_DISCARD`
   through).

5. **Overlay assembly** (`overlay::assemble`):
   - **CA tmpfs** (`ca::prepare_overlay`): if the `caCert` field is
     non-empty, the stage creates a small tmpfs at `/mnt/ca-overlay`
     with a per-distro CA bundle — the image's own bundle (read from
     the topmost layer that ships it) with the project CA appended.
     It then splices the tmpfs on top of the image lowerdir stack, so
     the CA is present without writes on the persistent upperdir.
     It also places the raw CA at distro anchor paths
